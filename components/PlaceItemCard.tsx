...
function PlaceItemCard({ place }:any) {
  return (
    <div className='w-full z-10 border-[1px] rounder-x1 shadow-md'>
      {place && place.photos && place.photos.length > 0 && (
        <Image src={BASE_URL_PHOTO + "&photo_reference=" + place?.photos[0]?.photo_reference + "&key=" + process.env.NEXT_PUBLIC_GOOGLE_PLACE_KEY}
          ...
          className="w-full h-[150px] object- rounded-t-xl" />
      )}
      <div className='p-1'>
        ...
        <div className='flex gap-2 mt-3'>
          ...
          <h2 className='text-[12px] text-gray-400 line-clamp-2'>{place.rating} ({place.user_ratings_total})</h2>
        </div>
